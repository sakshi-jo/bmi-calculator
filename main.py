...
def calculate_bmi():
    try:
        ...
        tag = "Underweight" if bmi < 18.5 else "Optimum Range" if bmi >= 18.5 and bmi < 24.9 else "Overweight" if bmi >= 25 and bmi < 29.9 else "Obese"

        result_label.configure(text= "Your BMI is {:.2f}".format(bmi))
        result_label2.configure(text="Your weight type is")
        result_label3.configure(text=tag)
...
#! MARK: Calculation
calculate_button = customtkinter.CTkButton(app, command=calculate_bmi, font=font2, text_color='#fff', text='Calculate BMI', fg_color='#06911f', hover_color='#0691aa', bg_color='#000', cursor='hand2', corner_radius=5, width=200 )
calculate_button.place(x=30, y=230)

result_label = customtkinter.CTkLabel(app, text='', font=font3, text_color='#fff', bg_color='#000')
result_label.place(x=30, y=280)

result_label2 = customtkinter.CTkLabel(app, text='', font=font2, text_color='#fff', bg_color='#000')
result_label2.place(x=30, y=350)

result_label3 = customtkinter.CTkLabel(app, text='', font=font3, text_color='#fff', bg_color='#000')
result_label3.place(x=30, y=380)
